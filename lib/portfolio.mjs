...
const HEADER_NAMES = {
  ...
  leftover: new Set(["leftover", "remaining", "remainingqty", "remainingquantity"]),
  spent: new Set(["spent", "cost", "total", "invested", "value"]),
};
...
function parseSummary(XLSX, workbook) {
  ...
  for (let index = start; index < rows.length; index++) {
    ...
    if (symbol) summarySymbols.add(symbol);
    const buy = numberOrNull(row?.[columns.buy]);
    if (!token || buy == null) continue;
    const qty = numberOrNull(row?.[columns.qty]) ?? 0;
    const leftover = columns.leftover == null ? null : numberOrNull(row?.[columns.leftover]);
    const currentQty = leftover ?? qty;
    // A numeric zero in Leftover explicitly marks a closed position.
  ...
  return summarySymbols.size ? { rows: output, summarySymbols } : null;
}
...
export function readPortfolioWorkbook(XLSX, workbook) {
  ...
  if (summary) {
    ...
    // Summary is authoritative for every symbol it contains, including closed
    // positions that were intentionally omitted from its current holdings.
    const genuinelyMissingBuys = buys.filter(
      (row) => !summary.summarySymbols.has(normalizeSymbol(row.token)),
    );
    return appendMissingBuyRows(summary.rows, genuinelyMissingBuys);
  }
