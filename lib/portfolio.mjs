...
function parseSummary(XLSX, workbook) {
  ...
  for (let index = start; index < rows.length; index++) {
    ...
    output.push({
      token,
      buy,
      qty: numberOrNull(row?.[columns.qty]) ?? 0,
      spent: columns.spent == null ? null : numberOrNull(row?.[columns.spent]),
    });
  }
  return output.length > 0 && output.every((row) => row.qty === 0) ? null : output;
}
...
export function readPortfolioWorkbook(XLSX, workbook) {
  ...
  if (summary?.length) {
    ...
    return appendMissingBuyRows(summary, buys);
  }
